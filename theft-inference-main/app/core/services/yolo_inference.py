...
class YoloInferenceService:
    ...
    def __init__(self, frame_queue: "Queue[Dict[str, Any]]", stop_event=None):
        ...
        # Setup ZMQ for sending results to theft detection
        self.context = zmq.Context()
        self.sender = self.context.socket(zmq.PUSH)
        self.sender.bind("tcp://*:5558")  # Bind to a port for sending results
    ...
    def run_yolo_inference(self, frame: np.ndarray) -> List[List[int]]:
        ...
        results = self.yolo_model(
            frame, verbose=False, classes=[1], conf=config.YOLO_CONFIDENCE_THRESHOLD
        )
    ...
    def process_frame(
        ...
        # Send results via ZMQ
        self.send_to_theft_detection(
    ...
    def send_to_theft_detection(
        ...
        Args:
            ...
            timestamp (float): The timestamp of the frame.
            detections (list): The YOLO detections.
            store_id (str): The store ID associated with the camera.
        """
        ...
        try:
            ...
            self.sender.send_pyobj(message, zmq.NOBLOCK)

            logger.info(
                f"Sent frame {frame_number} from camera {camera_id} with {len(detections)} detections"
            )

        except zmq.Again:
            # Queue full, drop frame
            logger.warning(
                f"YOLO Processor: ZMQ queue is full, dropping frame for {camera_id} (frame {frame_number})"
            )
            pass

        except Exception as e:
            logger.error(f"Failed to send message via ZMQ: {e}")

    def cleanup(self):
        """Cleans up resources."""
        self.sender.close()
        self.context.term()
        logger.info("YOLO Processor: Cleaned up ZMQ resources.")
